...
// 環境変数チェック
if (!SPREADSHEET_ID) {
  console.error('VITE_SPREADSHEET_ID is not set. Please check your environment variables.');
}
...
const fetchWithAuth = async (url, options = {}) => {
  // 環境変数チェック
  if (!SPREADSHEET_ID) {
    throw new Error('スプレッドシートIDが設定されていません。環境変数を確認してください。');
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      const errorMessage = error.error?.message || `APIリクエストが失敗しました (${response.status})`;
      
      // 認証エラーの場合
      if (response.status === 401 || response.status === 403) {
        localStorage.removeItem('google_access_token');
        localStorage.removeItem('google_token_expiry');
        throw new Error('認証が期限切れです。ページを更新して再ログインしてください。');
      }
